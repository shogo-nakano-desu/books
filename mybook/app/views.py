...
from app.forms import BookForm,ImpressionForm
...
def book_edit(request, book_id=None):
    ...
    return render(request, 'app/book_edit.html', dict(form=form, book_id=book_id))
...
class ImpressionList(ListView):
    ...
    template_name='app/impression_list.html'
...
def impression_edit(request,book_id,impression_id=None):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid(): # フォームのバリデーション
            ...
            impression.save()
            return redirect('app:impression_list', book_id=book_id)
    else: #GETの時
        form = ImpressionForm(instance=impression) # impression インスタンスからフォームを作成

    return render(request,
                  'app/impression_edit.html',
